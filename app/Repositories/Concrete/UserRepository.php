...
class UserRepository implements IUserRepository {
	 ...
	 public function getAdminData(){
		...
		foreach($users as $user){
			...
			if(array_key_exists('users', $_POST['schema'])){
				...
				foreach($_POST['schema']['users'] as $field){
					$val = (empty($user->$field) || $user->$field=='0000-00-00 00:00:00')? '-' : (string)$user->$field;
					
					if($field=='completed_profile' && $val==0):
						$val = 'FALSE';
					elseif($field=='completed_profile' && $val==1):
						$val = 'TRUE';
					endif;
	...
	public function getFilteredUsers($role,$search_term,$filters,$pageNumber,$orderBy,$sortOrder,$pagination){
		...
		$admins= ($pagination==1)? $query->paginate(20):$query->get();
	...
	public function AddOrUpdateProvider($request){
		...
		$isUpdate=false;
	 	...
	 	if(property_exists($provider, 'password') && $provider->password){
	 	...
	 	if(!$isUpdate){
				...
				if($move){
					...
					foreach($settings as $setting){
						...
						if($setting->slug=='thumbnail_image'){
							$newpath = storage_path() . '/profiles/' . $user->id . "/thumbnail_" . $profile_pic->getClientOriginalName();
						}
						elseif($setting->slug=='medium_image'){
							$newpath = storage_path() . '/profiles/' . $user->id . "/medium_" . $profile_pic->getClientOriginalName();
						}
						elseif($setting->slug=='large_image'){
							$newpath = storage_path() . '/profiles/' . $user->id . "/large_" . $profile_pic->getClientOriginalName();
						}
					...
					/*	Creating Images of different sizes end	*/
			...
			$user->save();
	 		...
	 		if(isset($profile_pic)){
				...
				$oldData='';
				$directoryPath = storage_path() . '/profiles/' . $user->id . "/";
				$path = $directoryPath . $oldPicName;
				...
				if($move){
					...
					$path = $directoryPath . $profile_pic->getClientOriginalName();
					/*	Creating Images of different sizes	*/
					foreach($settings as $setting){
						$size = explode('x',$setting->value);
						if($setting->slug=='thumbnail_image'){
