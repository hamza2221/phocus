        ...
        .then(function(response) {
			...
			else
			{
				if($scope.showAll){
					...
					$scope.limit=response.data.last_page;
					$scope.range=[];
					console.log(response.data.total/20);
					if((response.data.total/20)>2 || (response.data.total/20)<=1){
						$scope.shouldShowAll=false;
					}
					else{
...
phocusControllers.controller('ContactRequestController', ['$scope', '$rootScope', 'NgTableParams', 'ContactRequest', '$location', '$routeParams','Flash','$http','$localStorage','ApiUrl',
 function ($scope, $rootScope, NgTableParams, ContactRequest, $location, $routeParams,Flash,$http,$localStorage,ApiUrl) {
    ...
    $scope.SearchRequest=function(pageNumber,orderBy,sortOrder,setNo){
    	...
    	$scope.tableHeaders=[
    		{'name':'Full Name','colName':'full_name','width':'20%','class':''},
    		{'name':'Phone Number','colName':'phone','width':'20%','class':''},
    		{'name':'Location','colName':'location','width':'20%','class':''},
    		{'name':'Zipcode','colName':'zip_code','width':'10%','class':''},
    		{'name':'Subject','colName':'subject','width':'20%','class':''},
    		{'name':'Is provider','colName':'is_provider','width':'10%','class':''},
    		{'name':'...','colName':'...','width':'5%','class':''}
    	];
		if(setNo===undefined){
    		setNo=$scope.setNo;
    	}
			...
			$scope.tableHeaders=[
    		{'name':'...','colName':'...','width':'5%','class':''},
    		{'name':'Message','colName':'message','width':'40%','class':''},
    		{'name':'Email','colName':'email','width':'40%','class':''}
    		];
    		$scope.setNo=setNo;
		}
		else{
		...
		angular.forEach($scope.tableHeaders,function(header){
			console.log(header.colName);
			if(header.colName==orderBy){
				header.class='acive-sort sort-heading';
			}
			else{
				header.class='header sort-heading';
			}
		});
 ...
 function ($scope, VersionAssociation, $location, $rootScope,$routeParams,$http,$localStorage,Flash,ApiUrl,$uibModal) {
    ...
    $scope.TakeAction=function(action){
    	switch(action){
    		...
    		case 'add':
    			if($location.path().indexOf('edit') > -1){
					$location.path('/paper_trail~version_association/new');
				}
				else{
					$scope.lead=undefined;
					var message=Flash.get();
    		...
    		case 'edit':
    			if($location.path().indexOf('new') > -1){
					$location.path('/paper_trail~version_association/edit/'+$rootScope.vAssociationId);
				}else{
					var message=Flash.get();
					SetMessage(message);
				}
    ...
    $scope.AddNewAssociation=function(action){
    	if($scope.association.id>0){
    		VersionAssociation.update({id:$scope.association.id},$scope.association,function(data){
    			if(action=='edit'){
					$rootScope.vAssociationId = data.id;
				}
				if(action=='add'){
					$rootScope.addFromEdit = 1;
				}
				
				Flash.set({Msg:'Version association successfully updated',Type:'Success'});
				$scope.TakeAction(action);
			});
    	}
    	else{
	    	VersionAssociation.save($scope.association,function(data){
	    		if($localStorage.redirect===undefined || $localStorage.redirect){
	    			...
	    			Flash.set({Msg:'Version association successfully created',Type:'Success'});
    				$scope.TakeAction(action);
	    		}
	    		else{
    	...
    	if($routeParams.associationID){
	    	VersionAssociation.get(
	    		...
	    		function(response){
					...
					if($rootScope.vAssociationId != undefined){
						$rootScope.vAssociationId=undefined;
						Flash.set({Msg:'Version association successfully created',Type:'Success'});
						$scope.TakeAction('edit');
					}
    ...
    $http({
			...
			.then(function(response) {
				...
				if(selectedID>0){
					...
					$scope.selectedVersion = 'PaperTrail::Version #'+selectedID;
				}
...
phocusControllers.controller('SettingController', ['$scope', '$rootScope', 'NgTableParams', 'Setting', '$location', '$routeParams', '$timeout','$http','$localStorage','Flash','ApiUrl','$route',
 function ($scope, $rootScope, NgTableParams, Setting, $location, $routeParams, $timeout,$http,$localStorage,Flash,ApiUrl,$route) {
	
	$rootScope.seletedSettings = [];
	$rootScope.active_item = "setting"; //it highlight sidebar current link
    $scope.filters=[];
    $scope.reverseSort = false;
    $scope.orderBy = 'id';
    $scope.showAll=false;
    ...
    $scope.setupPage=function(){
        if(!$routeParams.adminID){
            $scope.mainHeadAddEdit='New Setting';
            $scope.breadHeadAddEdit='New';
            $scope.tabHeadAddEdit='Add new';
        }
        else{
            $scope.mainHeadAddEdit='Edit Setting';
            $scope.breadHeadAddEdit='Edit';
            $scope.tabHeadAddEdit='Edit';
	...
	$scope.exportFound=function(){
		...
		if($scope.searchPerformed ){
			...
			$location.path('/setting/export');
		}
		
	}
	
    $scope.Search=function(pageNumber,orderBy,sortOrder){
		if(pageNumber == -1){
			$scope.searchPerformed = true;
		}
        ...
        if (orderBy === undefined) {
        	orderBy = ($scope.orderBy != 'id')? $scope.orderBy:'id';
        }
        else{
            $scope.orderBy=orderBy;
        }
        ...
        .then(function(response) {
			...
			if($scope.exportFoundData)
			{
				...
				angular.forEach(response.data, function (row) {
					$scope.seletedSettings.push(row.id);
				});
    ...
    $scope.GetSettingByIDs=function(){
        $scope.seletedSettings=$localStorage.seletedSettings;
        //console.log($scope.seletedSettings);
        $http({
            url: ApiUrl+'setting/getbyids',
            method: "POST",
            data: { 'ids' : $scope.seletedSettings }
        })
        .then(function(response) {
            // success
			console.log(response);
    ...
    $scope.getLeads=function(){
        $scope.rows=Lead.query();
    };

    $scope.deleteSetting=function($settingID){
        $scope.setting=Setting.delete({id:$settingID},function(data){
            Flash.set({Msg:'Setting Deleted Successfully!',Type:'Success'});
            //console.log(Flash.queue);
            $location.path('/setting');
    });
    };
    ...
    $scope.DeleteByIds=function(){
        $scope.seletedSettings=$localStorage.seletedSettings;
        $http({
            url: ApiUrl+'setting/deletebyids',
            method: "POST",
            data: { 'ids' : $scope.seletedSettings }
        })
        .then(function(response) {
    ...
    $scope.GetSelectedToDelete=function(){
        ...
        $localStorage.seletedSettings=$scope.seletedSettings;
        $location.path('/setting/delete_bulk');
    ...
    $scope.TakeAction=function(action){
    	switch(action){
    		...
    		case 'edit':
				if($location.path().indexOf('new') > -1){
					$location.path('/setting/edit/'+$rootScope.settingId);
				}else{
					var message=Flash.get();
					SetMessage(message);
				}
    ...
    $scope.AddNew=function(action){
        if($scope.setting.id > 0){
            Setting.update({id:$scope.setting.id},$scope.setting,function(response){
				$scope.settingId = response.id;
				
				if(action=='add'){
					$rootScope.addFromEdit = 1;
				}
				
            	Flash.set({Msg:'Setting successfully updated',Type:'Success'});
                $scope.TakeAction(action);
        });
        }
        else{
            Setting.save($scope.setting,function(response){
				$rootScope.settingId = response.id;
                Flash.set({Msg:'Setting successfully created',Type:'Success'});
                $scope.TakeAction(action);
            });
        }
    };
    ...
    $scope.GetSetting=function(){
        $scope.setting=Setting.get({id:$routeParams.settingID},function(response){
			console.log(response);
			
			if($rootScope.settingId != undefined){
				$rootScope.settingId=undefined;
				Flash.set({Msg:'Setting successfully created',Type:'Success'});
				$scope.TakeAction('edit');
			}
...
phocusControllers.controller('SettingExportController', ['$scope', 'Setting', function ($scope, Setting) {
	$scope.CSRF_TOKEN = csrf_token_js;
	$scope.inputSelectedLeads = $scope.seletedSettings;
	
	/*	Admin Expert Checkboxes	*/
	$scope.Fields = [{ Name: "schema[setting][]", Value: 'id', title: 'Id'},
						  { Name: "schema[setting][]", Value: 'label', title: 'Label'},
						  { Name: "schema[setting][]", Value: 'value', title: 'Value'},
						  { Name: "schema[setting][]", Value: 'created_at', title: 'Created at'},
						  { Name: "schema[setting][]", Value: 'updated_at', title: 'Updated at'}];
...
phocusControllers.controller('PackageController', ['$scope', '$rootScope', 'NgTableParams', 'Package', '$location', '$routeParams', '$timeout','$http','$localStorage','Flash','ApiUrl','$route',
 function ($scope, $rootScope, NgTableParams, Package, $location, $routeParams, $timeout,$http,$localStorage,Flash,ApiUrl,$route) {
	
	$rootScope.seletedPackages = [];
	$rootScope.active_item = "package"; //it highlight sidebar current link
    $scope.filters=[];
    $scope.reverseSort = false;
    $scope.orderBy = 'id';
    $scope.showAll=false;
    ...
    $scope.setupPage=function(){
        if(!$routeParams.adminID){
            $scope.mainHeadAddEdit='New Package';
            $scope.breadHeadAddEdit='New';
            $scope.tabHeadAddEdit='Add new';
        }
        else{
            $scope.mainHeadAddEdit='Edit Package';
            $scope.breadHeadAddEdit='Edit';
            $scope.tabHeadAddEdit='Edit';
    ...
    $scope.Search=function(pageNumber,orderBy,sortOrder){
        ...
        .then(function(response) {
			...
			if($scope.exportFoundData)
			{
				...
				angular.forEach(response.data, function (row) {
					$scope.seletedPackages.push(row.id);
				});
    ...
    $scope.GetSettingByIDs=function(){
        $scope.seletedPackages=$localStorage.seletedPackages;
        //console.log($scope.seletedPackages);
        $http({
            url: ApiUrl+'package/getbyids',
            method: "POST",
            data: { 'ids' : $scope.seletedPackages }
        })
        .then(function(response) {
            // success
			console.log(response);
    ...
    $scope.deleteSetting=function($settingID){
        $scope.package=Package.delete({id:$settingID},function(data){
            Flash.set({Msg:'Package Deleted Successfully!',Type:'Success'});
            //console.log(Flash.queue);
            $location.path('/package');
		});
    };
    ...
    $scope.DeleteByIds=function(){
        $scope.seletedPackages=$localStorage.seletedPackages;
        $http({
            url: ApiUrl+'package/deletebyids',
            method: "POST",
            data: { 'ids' : $scope.seletedPackages }
        })
        .then(function(response) {
    ...
    $scope.GetSelectedToDelete=function(){
        ...
        $localStorage.seletedPackages=$scope.seletedPackages;
        $location.path('/package/delete_bulk');
    ...
    $scope.TakeAction=function(action){
    	switch(action){
    		...
    		case 'edit':
				if($location.path().indexOf('new') > -1){
					$location.path('/package/edit/'+$rootScope.settingId);
				}else{
					var message=Flash.get();
					SetMessage(message);
				}
    ...
    $scope.AddNew=function(action){
        if($scope.package.id > 0){
            Package.update({id:$scope.package.id},$scope.package,function(response){
				$scope.packageId = response.id;
				
				if(action=='add'){
					$rootScope.addFromEdit = 1;
				}
				
            	Flash.set({Msg:'Package successfully updated',Type:'Success'});
                $scope.TakeAction(action);
        });
        }
        else{
			var photo=picture.files.item(0);
            var jsonData=$scope.package;
		    ...
		    $http({
				...
				transformRequest: function (data) {
					var formData = new FormData();
					formData.append("model", angular.toJson($scope.package));
					formData.append("photo", picture.files.item(0));
					return formData;
				},
				data: { 'package':jsonData,'photo':photo },
            })
			.then(function(response) {
				console.log(response);
				/*if(response.data.errors)
				...
				{*/
					/*$rootScope.PackageId = response.data.id;
					Flash.set({Msg:'Package successfully created',Type:'Success'});
					$scope.TakeAction(action);*/
				//}
			});
			
			$scope.package.photo=picture.files.item(0);
            Package.save($scope.package,function(response){
				console.log(response);
				/*$rootScope.settingId = response.id;
                Flash.set({Msg:'Package successfully created',Type:'Success'});
                $scope.TakeAction(action);*/
    ...
    $scope.GetSetting=function(){
        $scope.package=Package.get({id:$routeParams.settingID},function(response){
			console.log(response);
